<?php
    ...
    /*if(isset($_POST['username'])) {
        ...
        $sql = "SELECT * FROM `logins` WHERE user='".$uname."' AND password='".$pwd."' limit 1";
    ...
    $error = '';
        if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['submit'])) {
            ...
            if (empty($error)) {
                if ($query = $db -> prepare("SELECT * FROM `logins` WHERE user_id = ?")) {
...
<body>
    ...
    <div class="login">
        ...
        <p><?php echo $error; ?></p>
        <form name="loginform" id="loginform" class="d-flex mx-auto loginform align-items-center flex-column" action="" method="post">
